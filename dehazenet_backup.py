...
class FogData(Dataset):
	IMAGE_LOADER = torchvision.transforms.Compose([
		transforms.ToTensor(),
...
class DehazeNetDriver:
	...
	def open(self, filename=r'defog4_noaug.pth'):
		self.net.load_state_dict(torch.load(filename, map_location='cpu'))
		return self

	def save(self, filename=r'defog4_noaug.pth'):
		torch.save(self.net.state_dict(), filename)
		return self
	
	@classmethod
	def main(open_dir='fogpic.jpg', save_dir='test.jpg'):
		img = Image.open(open_dir)
		img1 = FogData.IMAGE_LOADER(img)
		...
		t_list = []
		driver = DehazeNetDriver(DehazeNet()).open(r'defog4_noaug.pth')
...
class DehazeNetTrainer(DehazeNetDriver):
